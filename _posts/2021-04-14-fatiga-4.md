...
<img src = '/images/posts/2021-03-14-fatiga-4/tabla T.3-4.5.1.1'>
...
$$\sigma_t = \sigma_W \ cdot \frac{5}{3 – 2 \dot \kappa} \leq 0,66 \ cdot \sigma_E \tag*{(1)}$$
...
$$\sigma_t$$ is limited in every case to $$0,66 \cdot \sigma_E$$

Para traducir esto a un código de Python emplearé tres clases diferentes:

- Una primera clase llamada `DataFrame` donde obtendré los valores para calcular las tensiones admisibles.
...
class Formulae(DataFrame):
    """
    Formulae for the permissible stresses for fatigue with tensile and 
...
> Como se ha indicado arriba, la expresión que defina la tensión de tracción es:

> $$\kappa \leq 0 \text{ para tracción: } \sigma_t = \sigma_W \ cdot \frac{5}{3 – 2 \dot \kappa} \leq 0,66 \ cdot \sigma_E$$
...
> $$\kappa \leq 0 \text{ para compresión: } \sigma_c = \sigma_W \ cdot \frac{2}{1 - \kappa}$$
...
> $$\kappa > 0 \text{ para tracción: } \sigma_t = \frac{\sigma_O}{1 - \left(1 - \dfrac{\sigma_O}{\sigma_{+1}}\right) \ cdot \kappa} \leq 0,66 \ cdot \sigma_E$$

> Además $$\sigma_0$$ = tensión de tracción para $$\kappa$$ = 0 se da en la fórmula (1), esto es: $$\sigma_0 = 1,66 \cdot \sigma_W$$

> Y $$\sigma_{+1}$$ = tensión de tracción para $$\kappa$$ = +1, esto es, la tensión última $\sigma_R$ dividida por el coeficiente de seguridad 1,33: $$\sigma_{+1} = 0,75 \cdot \sigma_R$$
...
> Las tensione $$\sigma_0$$ y $$\sigma_{+1}$$ están definidas en lso siguientes métodos de la clase:
...
> $$\kappa > 0 \text{ para compresión: } \sigma_c = 1,2 \ cdot \sigma_t$$
...
Límite elástico $$\sigma_E = 355 N/mm²$$

Tensión de rotura $$\sigma_R = 490 N/mm²$$

Grupo de la compnente: E8

Efecto de entalladura: K3

Tensión máxima en la dirección x $$\sigma_{x,max} = -130 N/mm²$$

Tensión mínima en la dirección x $$\sigma_{x,min} = -59 N/mm²$$

(Recordad que las tensiones máxima y mínima se definen en términos de valores absolutos).

$$\kappa = \frac{-59}{-130} = 0,454 > 0$$

Tensión básica según tabla T3-4.5.1.1 $$\sigma_W =  45,0 N/mm²$$

$$\sigma_0 = 1,66 \cdot\ \sigma_W = 1,66 \cdot 45,0 = 74,7 \text{ N/mm²}$$

$$\sigma_{+1} = 0,75 \cdot\ \sigma_R = 0,75 \cdot 490 = 367,5 \text{ N/mm²}$$

$$\sigma_t = \frac{\sigma_O}{1 - \left(1 - \dfrac{\sigma_O}{\sigma_{+1}}\right) \ cdot \kappa} = \frac{74,7}{1 - \left(1 - \dfrac{74,7}{367,5}\right) \cdot 0,454} = 117,03 N/mm² \leq 0,66 \cdot \sigma_E = 0,66 \cdot 355 = 234,3 N/mm²$$

$$\sigma_c = 1,2 \cdot\ \sigma_t = 1,2 \cdot\ 117,03 = 140,44 N/mm²$$

Al ser $$\sigma_c$$ una tensión de compresión toma el valor negativo: $$\sigma_c = -140,44 N/mm²$$

Los valores $$\sigma_t = 117,03 N/mm²$$ y $$\sigma_c = -140,44 N/mm²$$ coinciden con los valores `sigma_tx_[MPa]` y `sigma_cx_[MPa]` del índice 0 del `DataFrame` `df`con los resultados. Si no coincidieran, o bien el código sería erróneo (tendría un bug), o bien el error estaría en la verificación.
...
Tensión máxima en la dirección x $$\sigma_{x,max} = -79 N/mm²$$

Tensión mínima en la dirección x $$\sigma_{x,min} = 69 N/mm²$$

$$\kappa = \frac{69}{-79} = -0,873 < 0$$

Tensión básica según tabla T3-4.5.1.1 $$\sigma_W = 33,3 N/mm²$$

$$\sigma_t = \sigma_W \frac{5}{3 – 2 \cdot \kappa} = 33,3 \frac{5}{3 - 2 \cdot (-0,873)} = 35,08 N/mm² \leq 0,66 \cdot \sigma_E = 0,66 \cdot 355 = 234,3 N/mm²$$

$$\sigma_c = \sigma_W \frac{2}{1 – \kappa} = 33,3 \frac{2}{1 - (-0,873)} = 35,56 N/mm² $$

Al ser $$\sigma_c$$ una tensión de compresión toma el valor negativo: $$\sigma_c = -35,56 N/mm²$$

Los valores $$\sigma_t = 35,08 N/mm²$$ y $$\sigma_c = -35,56 N/mm²$$ coinciden en este caso también con los valores `sigma_tx_[MPa]` y `sigma_cx_[MPa]` del índice 2 del `DataFrame` `df`con los resultados. 
