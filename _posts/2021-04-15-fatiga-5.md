...
La segunda clase `Formulae` hereda de la clase `DataFrame` los atributos para operar e inicializa las variables de instancia del límite elástico $$\sigma_E$$ y la tensión de rotura $$\sigma_R$$.
...
En este caso de la tensión de cortadura, habrá que considerar $$\sigma_W = \sigma_W_0$$
...
cols = ['bar', 'node', 'component_group', 'notch_effect', 'tau_a_[MPa]']
df[cols]
```
...
Como era de esperar, el valor $$\tau_a = 91,00N/mm²$$ coincide con el valore `tau_a_[MPa]` del índice 2 del `DataFrame df` con los resultados. Si no lo hiciera, habría que buscar el error, bien en el código, bien en la comprobación.
